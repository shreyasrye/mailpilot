...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    handlers=[
                        logging.FileHandler(os.path.join(log_directory, "fetch_emails.log")),
                    ])

logger = logging.getLogger(__name__)

SCOPES = ["https://www.googleapis.com/auth/gmail.readonly"]
with open("/app/app/config.json") as config_file:
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                creds = None
        if not creds:
            flow = InstalledAppFlow.from_client_secrets_file("/app/app/client_secret.json", SCOPES)
